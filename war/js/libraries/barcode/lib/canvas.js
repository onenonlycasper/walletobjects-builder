...
(function() {
	...
	for (var i = 0; i < symdesc.length; i++) {
		if (!symdesc[i]) // last entry can be null
			continue;
		emit('js/libraries/barcode/bwipp/' + symdesc[i].sym);
	}

	// Renderers
	emit('js/libraries/barcode/bwipp/renlinear');
	emit('js/libraries/barcode/bwipp/renmatrix');
	emit('js/libraries/barcode/bwipp/renmaximatrix');

	// Fonts
	for (var x = 1; x <= 10; x++) {
		emit('js/libraries/barcode/fonts/ocrb10-' + (x < 10 ? '0' : '') + x);
		emit('js/libraries/barcode/fonts/ocrb12-' + (x < 10 ? '0' : '') + x);
	}
})();
...
function Bitmap() {
	...
	this.show = function(element, rot) {
		// Small change so that we may render the barcode before the items are shown
		var cvs = element; //document.getElementById(cvsid);
		if (pts.length == 0) {
			cvs.width = 32;
			cvs.height = 32;
